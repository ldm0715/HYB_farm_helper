...
- 展示金额 = 接口价格整数 / `500000`
- 代码常量：`const PRICE_DIVISOR = 500000`
- 界面金额固定展示 2 位小数
...
- 原始路径：`/farm/crops/pumpkin`
- 拼接规则：`https://cdk.hybgzs.com` + `seedImage` + `_s4.png`
- 完整示例：`https://cdk.hybgzs.com/farm/crops/pumpkin_s4.png`
...
| `seeds[].id` | 作物 ID |
| `seeds[].name` | 作物名称 |
| `seeds[].image` | 作物图标相对路径 |
| `seeds[].growthTime` | 成熟时间，单位秒 |
| `seeds[].harvestQuantity` | 单次成熟产量 |
| `seeds[].isVipOnly` | 是否 VIP 作物 |
...
| `skipped` | 无需处理的地块数 |
| `energySpent` | 消耗的精力值 |
| `byKind.thirsty` | 缺水 debuff 处理数量 |
| `byKind.weed` | 杂草 debuff 处理数量 |
| `byKind.pest` | 虫害 debuff 处理数量 |
...
| `seedId` | 作物 ID |
| `quantity` | 卖出数量 |
| `expectedUnitPrice` | 来自回收报价接口 `unitPrice` |
...
| --- | --- |
| `friend.id` | 好友 ID |
| `friend.username` | 好友名称 |
| `friend.avatar` | 好友头像 |
| `crops[].plotIndex` | 地块序号 |
| `crops[].seedName` | 第一块地作物名称 |
| `crops[].seedImage` | 第一块地作物图标 |
| `crops[].maturesAt` | 第一块地 UTC 成熟时间 |
| `crops[].isMature` | 第一块地是否成熟 |
| `crops[].remainingTime` | 第一块地剩余成熟秒数 |
