...
(function () {
  ...
  /**
   * 重构预览版目标：
   * 1. 不引入构建步骤，保持当前用户脚本可直接安装。
   * 2. 修复审计报告中已确认的稳定性问题。
   * 3. 将高风险的好友详情扇出请求和状态栏文案收敛到专门的辅助函数。
   *
   * 等项目加入 Tampermonkey 分发用的打包步骤后，可以继续把 API 客户端、归一化器、
   * 操作流程和渲染辅助函数拆到独立文件。
   */
   ...
   * 对数组执行有限并发异步映射。
   *
   * 好友详情接口属于扇出请求，不能因为好友数量变大就一次性打满请求，也不能让单个失败拖垮整页。
   *
   * @param {Array} items 要处理的输入数组。
   * @param {number} limit 最大并发数。
   * @param {Function} mapper 异步映射函数。
   * @returns {Promise<Array<object>>} 每一项的 fulfilled/rejected 结果。
   */
  ...
  /**
   * 拉取好友列表，并以有限并发获取每个好友的农场详情。
   *
   * 单个好友详情失败时保留其它成功结果，并由 `friendStatusFailures` 提示失败数量。
   *
   * @returns {Promise<object>} 好友状态和详情失败数量。
   */
  async function fetchFriendStatuses() {
    ...
    const detailResults = await mapLimitSettled(
      friends,
      FRIEND_DETAIL_CONCURRENCY,
      async (friend) => {
        const detailPayload = await requestJson(buildFriendFarmUrl(friend.id));
        return normalizeFriendFarm(friend, detailPayload);
      },
    );
    const statuses = detailResults
      .filter((result) => result.status === "fulfilled")
      .map((result) => result.value);
    const failureCount = detailResults.length - statuses.length;
    ...
    return {
      friends: statuses,
      friendStatusFailures: failureCount,
    };
  }
   ...
   * 生成顶部状态栏文案。
   *
   * 后台成熟提醒刷新失败不阻断页面，但需要留下可见诊断信号，避免用户误以为提醒仍然可靠。
   *
   * @returns {string} 状态栏文案。
   */
  function getStatusText() {
    if (state.updatedAt) {
      return state.lastBackgroundRefreshErrorAt
        ? `更新于 ${state.updatedAt} · 后台刷新失败 ${state.lastBackgroundRefreshErrorAt}`
        : `更新于 ${state.updatedAt}`;
    }
    ...
    return state.lastBackgroundRefreshErrorAt
      ? `展开后加载实时数据 · 后台刷新失败 ${state.lastBackgroundRefreshErrorAt}`
      : "展开后加载实时数据";
  }
  ...
  function render(api) {
    ...
    api.status.textContent = getStatusText();
  ...
  function renderFriendsPage(api) {
    ...
    const partialFailureHtml =
      state.friendStatusFailures > 0
        ? `<div class="steal-notice error">有 ${formatNumber(
            state.friendStatusFailures,
            0,
          )} 位好友状态加载失败，其余好友已正常显示。</div>`
        : "";
    ...
    if (friends.length === 0) {
      api.body.innerHTML =
        state.friendStatusFailures > 0
          ? `<div class="error">好友状态加载失败 ${formatNumber(
              state.friendStatusFailures,
              0,
            )} 个，请稍后刷新重试。</div>`
          : `<div class="empty">暂无好友农场数据</div>`;
      return;
    }
  ...
  async function handleStealFriend(api, friendId) {
    ...
    try {
      ...
      let nextFriendStatusFailures = state.friendStatusFailures;

      try {
        // 成功后尽量刷新好友状态，让已偷过的好友从“可偷菜”里退出来。
        const nextFriendState = await fetchFriendStatuses();
        nextFriends = nextFriendState.friends;
        nextFriendStatusFailures = nextFriendState.friendStatusFailures;
      } catch {
      ...
      state = {
        ...
        friends: nextFriends,
        friendStatusFailures: nextFriendStatusFailures,
        updatedAt: new Date().toLocaleTimeString("zh-CN", {
          hour: "2-digit",
          minute: "2-digit",
          second: "2-digit",
  ...
  async function refreshCropStatus(api) {
    ...
    try {
      ...
      state = {
        ...state,
        cropStatusLoading: false,
        lastBackgroundRefreshErrorAt: new Date().toLocaleTimeString("zh-CN", {
          hour: "2-digit",
          minute: "2-digit",
          second: "2-digit",
        }),
