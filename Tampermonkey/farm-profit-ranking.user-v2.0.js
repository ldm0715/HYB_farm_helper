...
// @version      2.0.0
// @description  轻量展示作物成熟信息、实时价格和收益排行。
// @author       Codex
  ...
  function createRoot() {
    ...
    shadow.innerHTML = `
      <style>
        ...
        .body {
          max-height: min(530px, calc(100vh - 230px));
          overflow: auto;
      ...
      <table>
        <thead>
          <tr>
            <th class="rank">排名</th>
            <th class="name">作物</th>
            <th class="seed-id">ID</th>
            <th class="time">成熟时间</th>
            <th class="num">数量</th>
            <th class="money">单价</th>
            <th class="money harvest">单次收益</th>
            <th class="money">每小时</th>
            <th class="vip">VIP</th>
          </tr>
        </thead>
        <tbody>
          ${rows
            .map(
              (row, index) => `
                <tr title="${escapeHtml(row.name)} - ${escapeHtml(row.id)}">
                  <td class="rank">${index + 1}</td>
                  <td class="name">${escapeHtml(row.name)}</td>
                  <td class="seed-id">${escapeHtml(row.id)}</td>
                  <td class="time">${formatGrowthTime(row.growthTimeSeconds)}</td>
                  <td class="num">${row.harvestQuantity}</td>
                  <td class="money">${formatUsd(row.unitPrice)}</td>
                  <td class="money harvest">${formatUsd(row.harvestRevenue)}</td>
                  <td class="money"><strong>${formatUsd(row.revenuePerHour)}</strong></td>
                  <td class="vip"><span class="badge ${row.isVipOnly ? "yes" : ""}">${
                    row.isVipOnly ? "是" : "否"
                  }</span></td>
                </tr>
              `,
